...
class DupeAskWindow:
    def __init__(self, allowDupes: BooleanVar, parent):
        ...
        self.dupeAsk.geometry(f'{int(parent.root.winfo_screenwidth()*.175)}x{int(parent.root.winfo_screenheight()*.1)}')
        ...
        dupeFrame.columnconfigure(0, weight=2)
        dupeFrame.columnconfigure(1, weight=1)
        dupeFrame.columnconfigure(2, weight=2)
        dupeFrame.rowconfigure(0, weight=1)
        dupeFrame.rowconfigure(1, weight=1)
        dupeFrame.grid(sticky=NSEW)

        dupeLabel = Label(dupeFrame, text = "You have selected duplicates of one or more song. Allow duplicates? (This will be remembered for this playlist.)", wraplength=self.dupeAsk.winfo_width()-20, anchor=CENTER)
        dupeLabel.grid(column = 0, row = 0, sticky = NSEW, columnspan=3)
        ...
        dupeAllowButton.grid(column = 2, row = 1, sticky = EW)
